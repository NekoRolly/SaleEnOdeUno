struct Graph {
    ...
    int lca(int u, int v, int n) { // O(n)
        ...
        while (true) {
            ...
            marked[u] = true;
            if (match[u] == -1) break; // we reach the root : unexposed vertex
            u = p[match[u]];
        }
        while (true) {
            v = base[v];
    ...
    void markPath(int u, int b, int children) {
        while (base[u] != b) {
            ...
            p[u] = children, children = match[u], u = p[match[u]];
        }
    }
    ...
    int findPath(int root, int n) {
        for (int i = 0; i < n; i++) base[i] = i, used[i] = false, p[i] = -1;
        used[root] = true;
        queue<int> q;
        q.push(root);
        while (!q.empty()) {
            ...
            for (int to : adj[u]) {
                ...
                if (to == root || (match[to] != -1 && p[match[to]] != -1))
                    compress(u, to, n, q); // we find a blossom
                else if (p[to] == -1) {
                    // augmenting path or continue bfs
                    p[to] = u;
                    if (match[to] == -1) return to; // we find an augmenting path
                    // we continue the bfs
            ...
            if (match[u] == -1) {
                ...
                if (v != -1) {
                    ...
                    while (v != -1) {
                        ...
                        match[v] = p[v], match[p[v]] = v, v = nextVertex;
                    }
                }
            }
        }
